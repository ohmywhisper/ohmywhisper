...
CGO_LDFLAGS_VAL := -L$(LIB_DIR) -Wl,--start-group -lwhisper -lggml -lggml-cpu -lggml-base -Wl,--end-group -lstdc++ -lgomp -lm -lpthread

ohmywhisper: whisper
	mkdir -p $(BIN_DIR)
	CGO_ENABLED=1 CGO_LDFLAGS="$(CGO_LDFLAGS_VAL)" $(GO) build -o $(BIN_DIR)/ohmywhisper ./cmd/
